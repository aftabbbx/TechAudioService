...
export function ProductFormModal({ product, categories = [], onClose, onSuccess }) {
  ...
  const [specs, setSpecs] = useState(() => {
    if (isEdit && product.specs?.length) {
      return product.specs.map((s) =>
        typeof s === "string" ? s : (s.label && s.value ? `${s.label}: ${s.value}` : String(s))
      );
    }
    return [""];
  });
  ...
  const updateSpec = (i, val) =>
    setSpecs((s) => s.map((spec, idx) => (idx === i ? val : spec)));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append specs as JSON array of strings
      const cleanSpecs = specs.filter((s) => String(s).trim() !== "");
      ...
      <div className="admin-modal admin-modal--wide">
        ...
        <div className="admin-modal-header">
          <h2 className="admin-modal-title">
            {isEdit ? `Edit — ${product.model || product.name}` : "Add New Product"}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="admin-modal-body">
          ...
          <div className="admin-form-grid">
            ...
            <div className="admin-form-group">
              ...
              <input
                ...
                value={form.model}
                onChange={handleChange}
                placeholder="e.g. DCA4000"
              />
            </div>

            {/* SKU */}
            <div className="admin-form-group">
              <label className="admin-label" htmlFor="pf-sku">SKU *</label>
              <input
                id="pf-sku"
                name="sku"
                type="text"
                className={`admin-input ${fieldErrors.sku ? "admin-input--error" : ""}`}
                value={form.sku}
                onChange={handleChange}
                placeholder="e.g. DCA4000"
              />
            ...
            {/* Brand */}
            <div className="admin-form-group">
              <label className="admin-label" htmlFor="pf-brand">Brand</label>
              <input
                id="pf-brand"
                name="brand"
                type="text"
                ...
                value={form.brand}
                onChange={handleChange}
                placeholder="e.g. AudioTechServices"
              />
            </div>
            ...
            <div className="admin-form-group">
              ...
              <input
                ...
                value={form.badge}
                onChange={handleChange}
                placeholder="e.g. Flagship, New, Best Seller"
              />
            </div>
          ...
          <p className="admin-section-label" style={{ marginTop: 8 }}>Specifications</p>
            ...
            {specs.map((spec, i) => (
              <div key={i} className="admin-spec-row">
                ...
                <input
                  ...
                  placeholder={`Spec #${i + 1} — e.g. 4 Channels`}
                />
                <button
                  type="button"
