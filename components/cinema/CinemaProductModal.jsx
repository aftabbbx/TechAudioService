    ...
    >
      ...
      >
        ...
        <div className="grid md:grid-cols-2">
          {/* Image */}
          <div className="flex items-center justify-center min-h-[280px] relative overflow-hidden" style={{ backgroundColor: "var(--surface-alt)" }}>
            {product.image ? (
              <img 
                src={product.image} 
                alt={product.name}
          ...
          <div className="p-8 md:p-10">
            <span className="inline-block text-xs font-bold uppercase tracking-wider mb-2" style={{ color: "var(--accent)" }}>
              {product.category}
            </span>
            <h2 className="text-sm font-mono mb-1" style={{ color: "var(--text-muted)" }}>{product.model}</h2>
            <h3 className="text-2xl font-bold mb-4" style={{ color: "var(--text)" }}>{product.name}</h3>
            ...
            <div className="mb-8">
              ...
              <div className="border rounded-lg overflow-hidden" style={{ borderColor: "var(--border)" }}>
                {product.specs.map((spec, i) => (
                  <div
                    key={i}
                    className="px-4 py-2.5 text-sm flex justify-between"
                    style={{
                      backgroundColor: i % 2 === 0 ? "var(--surface-alt)" : "white",
                    }}
                  >
                    <span className="font-medium" style={{ color: "var(--text)" }}>{spec.label}</span>
            ...
            <div className="flex flex-wrap gap-3">
              <a
                href="/contact"
                className="inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold rounded-lg text-white transition-all"
